...
def act(self, game_state: dict) -> str:
    ...
    if self.train and random.random() < random_prob:
        ...
        action = ACTIONS[torch.multinomial(prediction, 1).item()]
        return action
...
def state_to_features(game_state: dict) -> list:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    if game_state is None:
        #return None
        return [torch.tensor([float('nan'), float('nan'), float('nan'), float('nan'), float('nan'), float('nan'), float('nan')], dtype=torch.float),
                torch.tensor([float('nan'), float('nan'), float('nan'), float('nan'), float('nan'), float('nan'), float('nan')], dtype=torch.float),
                torch.tensor([float('nan'), float('nan'), float('nan'), float('nan'), float('nan'), float('nan'), float('nan')], dtype=torch.float),
                torch.tensor([float('nan'), float('nan'), float('nan'), float('nan'), float('nan'), float('nan'), float('nan')], dtype=torch.float)]
    else:
