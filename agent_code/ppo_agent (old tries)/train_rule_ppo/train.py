...
class SimpleNet(nn.Module):
    def __init__(self, input_sizes, hidden_layer_sizes, final_hidden_layer_size, output_size):
        ...
        self.intermediate_layers=nn.Linear(hidden_layer_sizes[len(hidden_layer_sizes)-1], final_hidden_layer_size)
        self.final_layers= nn.Linear(final_hidden_layer_size, output_size)
        self.critic_layers= nn.Sequential(
            nn.Linear(final_hidden_layer_size, hidden_layer_sizes[1]), 
            nn.ReLU(), 
    ...
    def forward(self, input_tensor):
        ...
        combined_output=self.hidden_layers(masked_input)
...
def setup_training(self):
    ...
    self.hidden_layer_sizes = [12, 24]  # Adjust hidden layer size
    self.output_size = len(ACTIONS)  # Number of output values
    self.final_hidden_layer_size=self.hidden_layer_sizes[len(self.hidden_layer_sizes)-1]*2
    self.model = SimpleNet(self.input_sizes, self.hidden_layer_sizes, self.final_hidden_layer_size, self.output_size)
