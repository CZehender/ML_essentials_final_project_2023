...
class MultiBranchNet(nn.Module):
    def __init__(self, input_sizes, hidden_layer_sizes, final_hidden_layer_size, output_size):
        ...
        self.critic_layers= nn.Linear(final_hidden_layer_size, 1)
...
def train(self, mini_sample):
    ...
    for old_state, action, reward, new_state in zip(old_states, actions, rewards, new_states):
        ...
        entropy_loss = -(action_probs * action_log_probs).sum(-1)
        entropy_loss_list.append(entropy_loss)

    loss = actor_loss_list.mean() + self.value_coeff * critic_loss_list.mean() - self.entropy_coeff * entropy_loss_list.mean()
