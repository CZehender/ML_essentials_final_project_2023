...
class MultiBranchNet(nn.Module):
    def __init__(self, input_sizes, hidden_layer_sizes, output_size):
        ...
        self.branches = nn.ModuleList([
            self.create_branch(input_size, hidden_layer_sizes[0])
            for input_size in input_sizes
        ])

        #Add a hidden layer to the final layer:
        self.final_layers=nn.Sequential(
            nn.Linear(len(input_sizes) * hidden_layer_sizes[0][len(hidden_layer_sizes[0])-1], hidden_layer_sizes[1][0]), 
            nn.ReLU(), 
            nn.Linear(hidden_layer_sizes[1][0], output_size))

    def create_branch(self, input_size, hidden_layer_sizes):
        layers = []
        prev_size = input_size
        for hidden_size in hidden_layer_sizes[0]:
...
def setup_training(self):
    ...
    self.input_sizes = [11, 11, 11]  # Size of each input np.array
    self.hidden_layer_sizes = [(16, 32),(256)]  # Adjust hidden layer size
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    #e.INVALID_ACTION ordangerous action:
    if (action == 'UP' and old_state[1][2]!=0 ) or (action == 'RIGHT' and old_state[1][1]!=0) or (action == 'DOWN' and old_state[1][3]!=0) or (action == 'LEFT' and old_state[1][0]!=0)  or ((action == 'BOMB' or action == 'WAIT') and old_state[1][10]!=0):
        add_reward=-42
    else:
        add_reward=16
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    if (e.KILLED_SELF in events) or (e.GOT_KILLED in events) and not check_explosion_kill(self, last_game_state, last_action):
        ...
        for i in range(len(self.memory) -1 - min(last, len(self.memory)), len(self.memory) - 1, 1):
            old_state, action, reward, new_state = self.memory[i]
            if (((np.sqrt((old_state[1][4])**2+(old_state[1][5])**2)<np.sqrt((new_state[1][4])**2+(new_state[1][5])**2) or new_state[1][10]==0) and old_state[1][10]==1) or (old_state[1][10]==0 and new_state[1][10]==0)):
                self.memory[i] = (old_state, action, reward + add_reward, new_state)
                self.game_reward += add_reward
            if (old_state[1][10]==1 and (np.sqrt((old_state[1][4])**2+(old_state[1][5])**2)>np.sqrt((new_state[1][4])**2+(new_state[1][5])**2) and not new_state[1][10]==0)) or (old_state[1][10]==0 and new_state[1][10]==1):
    ...
    #final_reward = action_counter
    # Modify the last `last_entries` entries in memory
    for i in range(len(self.memory) -1 - min(last_entries, len(self.memory)), len(self.memory) -1, 1):
        old_state, action, reward, new_state = self.memory[i]
        if reward>=0:
...
def train_BOMB_EXPLODED (self, old_game_state: dict, new_game_state: dict, old_state: list, new_state: list):
    for i, ((x, y), c) in enumerate(old_game_state['bombs']):
        ...
        if np.sqrt((old_game_state ['self'][3][0]-bomb_x)**2+(old_game_state ['self'][3][1]-bomb_y)**2)<=4:
            ...
            for i in range(len(self.memory) -1 - min(last, len(self.memory)), len(self.memory) -1, 1):
                old_state, action, reward, new_state = self.memory[i]
                #if old_state[1][10]==0 and new_state[1][10]==0:
                #if reward>=0:
                if (old_state[1][10]==1 and np.sqrt((old_state[1][4])**2+(old_state[1][5])**2)<np.sqrt((new_state[1][4])**2+(new_state[1][5])**2)) or (old_state[1][10]==1 and new_state[1][10]==0):
                    self.memory[i] = (old_state, action, reward + add_reward, new_state)
                    self.game_reward += add_reward
            #print("Memory length:", len(self.memory))
            if len(self.memory) > last:
...
def train_KILLED_OPPONENT (self, new_game_state: dict, old_state: list, new_state: list):
    ...
    for i in range(len(self.memory) -1 - min(last, len(self.memory)), len(self.memory) -1, 1):
        old_state, action, reward, new_state = self.memory[i]
        if not (old_state[1][10]==1 and (action== 'BOMB' or action== 'WAIT')):
            self.memory[i] = (old_state, action, reward + add_reward, new_state)
            self.game_reward += add_reward
            if action==4:
                self.memory[i] = (old_state, action, reward + add_reward+6, new_state)
                self.game_reward += add_reward + 6
    for i in range(len(self.memory) -1 - min(last, len(self.memory)), len(self.memory) -1 - 5, 1):
        old_state, action, reward, new_state = self.memory[i]
        if not (old_state[1][10]==1 and (action== 'BOMB' or action== 'WAIT')):
...
def train_BOMB_DROPPED (self, old_game_state: dict, new_game_state: dict, old_state: list, new_state: list):
    ...
    if counter>0:
        ...
        for i in range(len(self.memory) -1 - min(last, len(self.memory)), len(self.memory) -1, 1):
            old_state, action, reward, new_state = self.memory[i]
            if np.sqrt((old_state[2][4])**2+(old_state[2][5])**2)>np.sqrt((new_state[2][4])**2+(new_state[2][5])**2) or np.sqrt((old_state[2][6])**2+(old_state[2][7])**2)>np.sqrt((new_state[2][6])**2+(new_state[2][7])**2):
                self.memory[i] = (old_state, action, reward + add_reward, new_state)
                self.game_reward += add_reward
        if len(self.memory) > last:
            batch = list(itertools.islice(self.memory, len(self.memory)-last, len(self.memory)))
...
def train_COIN_FOUND (self, new_game_state: dict, old_state: list, new_state: list):
    ...
    batch = list(itertools.islice(self.memory, len(self.memory)-5, len(self.memory)-4))
    train(self, batch)
