...
def train(self, mini_sample):
    ...
    for old_state, action, reward, new_state in zip(old_states, actions, rewards, new_states):
        ...
        new_q_values = self.target_model(new_state)
    ...
    if self.n_games%self.target_update==0:
    ...
    '''#Used for plotting for the first game due to an exploding loss
    if intermediate_loss<=1: 
        self.game_loss=((self.game_loss[1]*self.game_loss[0]+intermediate_loss)/(self.game_loss[1]+len(mini_sample)),self.game_loss[1]+len(mini_sample))
    else:
        intermediate_loss=intermediate_loss/intermediate_loss
        self.game_loss=((self.game_loss[1]*self.game_loss[0]+intermediate_loss)/(self.game_loss[1]+len(mini_sample)),self.game_loss[1]+len(mini_sample))'''
...
def setup_training(self):
    ...
    self.game_reward=0
    self.lr=0.01
    self.target_update=10 #How often the target network is updated
    ...
    self.target_model = MultiBranchNet(self.input_sizes, self.hidden_layer_sizes, self.final_hidden_layer_size, self.output_size, self.slope)
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    #print(" Action: ", self.memory[len(self.memory)-1][1], " Reward: ", self.memory[len(self.memory)-1][2])
    ...
    if len(self.memory)>=9:
        self.normalizer.append(self.memory[len(self.memory)-9][2]) 
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    self.way.append((last_game_state['self'][3], last_game_state['self'][3]))
...
def train_COIN_COLLECTED (self, new_game_state: dict, old_state: list, new_state: list):
    max_coins=50
    if 'coin-heaven' in self.command:
        max_coins=50
    else:
        max_coins=9
    #print(max_coins)
    ...
    add_reward=(max_coins-(10-math.e**(-0.075*(new_game_state['self'][1])-22.97)))/(2.5-math.e**(-0.075*(self.interstep_counter-6.4)))*(75/max_coins)
    #print(add_reward)
...
def train_KILLED_OPPONENT (self, new_game_state: dict, old_state: list, new_state: list):
    ...
    for i in range(len(self.memory) - min(last, len(self.memory)), len(self.memory), 1):
        ...
        if not (old_state[0][4]==1 and (action== 4 or action== 5)):
            ...
            self.game_reward += add_reward
            if action==4:
                self.memory[i] = (old_state, action, reward + add_reward+18, new_state)
                self.game_reward += add_reward + 18
