...
def state_to_features(game_state: dict) -> torch.tensor:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    state = [
    # Alle vier Richtungen stone wall, crate, bomb, explosion, other agent, coin
    feature1(x_p, y_p, game_state),
    
    # own score, step, bomb possibility, distance to wall
    torch.tensor([game_state['self'][1], game_state['step'], int(game_state['self'][2]), distance_to_wall(x_p, y_p)], dtype=torch.float),
...
def calculate_gravD_coins (own_x: int, own_y: int, game_state: dict) -> torch.tensor:
    if len(game_state['coins'])>0:
        ...
        return torch.tensor([own_x-center_of_gravity_x, own_y-center_of_gravity_y, density, float(num_obj)], dtype=torch.float)
    else:
...
def feature1(x_p: int, y_p: int, game_state: dict) -> torch.tensor:
    return torch.tensor([check_tile(x_p-1, y_p, game_state), check_tile(x_p+1, y_p, game_state), check_tile(x_p, y_p+1, game_state), check_tile(x_p, y_p-1, game_state)], dtype=torch.float)