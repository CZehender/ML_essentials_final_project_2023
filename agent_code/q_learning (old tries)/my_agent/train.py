...
def train_BOMB_DROPPED (self, new_game_state: dict, old_state: list, new_state: list):
    if (old_state[0][0]==-2.) or (old_state[0][1]==-2.) or (old_state[0][2]==-2.) or (old_state[0][3]==-2.0):
        last=5
        add_reward=12
...
def train_COIN_FOUND (self, new_game_state: dict, old_state: list, new_state: list):
    ...
    batch = list(itertools.islice(self.memory, len(self.memory)-4, len(self.memory)-3))
    train(self, batch)
