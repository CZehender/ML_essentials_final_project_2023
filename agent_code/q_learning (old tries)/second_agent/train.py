...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    with open(output_file_path, "a") as output_file:
        output_file.write(f"Game {n_games}, Mean Loss {self.game_loss[0]}, Actions {action_counter}, Mean reward {mean_reward}, Final reward {final_reward},  Ungefährer reward {self.game_reward}, Record {record}\n")
...
def train_COIN_COLLECTED (self, new_game_state: dict, old_state: list, new_state: list):
    ...
    self.game_reward += last*add_reward
    # Modify the last "last" entries in memory
