...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    #e.INVALID_ACTION or dangerous action: #left, #right, #up, #down, #wait, #bomb
    if (action == 'UP' and old_state[0][2]!=0 ) or (action == 'RIGHT' and old_state[0][1]!=0) or (action == 'DOWN' and old_state[0][3]!=0) or (action == 'LEFT' and old_state[0][0]!=0)  or ((action == 'BOMB' or action == 'WAIT') and old_state[0][4]!=0) or (action == 'BOMB' and old_state[0][5]!=1):
        add_reward=-100
    else:
        add_reward=100
