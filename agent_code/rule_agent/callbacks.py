...
def act(self, game_state: dict) -> str:
    ...
    if self.train and random.random() < random_prob:
        ...
        prediction, _ = self.model(c_state) 
        ...
        return action'''
        act=ACTIONS[torch.argmax(state_to_features(game_state)).item()] 
        print(act)
        return act 
...
def bomb_sense(own_x: int, own_y: int, game_state: dict) -> float:
    ...
    crate_counter=0
    stone_counter=0
    if game_state['field'][own_x+1][own_y] == 1:
        crate_counter+=1
    if game_state['field'][own_x][own_y-1] == 1:
        crate_counter+=1
    if game_state['field'][own_x][own_y+1] == 1:
        crate_counter+=1
    if game_state['field'][own_x-1][own_y] == 1:
    ...
    if ((game_state['self'][2])==True and viability_check(own_x, own_y, game_state)==1 and (check_kill_likelyhood(own_x, own_y, game_state)==0 or check==1)):
        bomb_sense=1
    elif (game_state['self'][2])==True and viability_check(own_x, own_y, game_state)==1 and ((crate_counter>=2 and stone_counter==1) or crate_counter>=3 or (crate_counter>=1 and stone_counter==2)) and np.sqrt((calculate_gravD_others(own_x, own_y, game_state)[0]) ** 2 + (calculate_gravD_others(own_x, own_y, game_state)[1]) ** 2)>=5:
        bomb_sense=1
    elif game_state['self'][2]==True and viability_check(own_x, own_y, game_state)==1 and math.isnan(calculate_gravD_others (own_x, own_y, game_state)[0]) and (crate_counter>=1):
        bomb_sense=1
    elif game_state['self'][2]==True and viability_check(own_x, own_y, game_state)==1 and (math.isnan(calculate_gravD_others (own_x, own_y, game_state)[0]) or np.sqrt((calculate_gravD_others(own_x, own_y, game_state)[0]) ** 2 + (calculate_gravD_others(own_x, own_y, game_state)[1]) ** 2)>=5)  and (crate_counter>=1 and stone_counter==2) and math.isnan(calculate_gravD_coins (own_x, own_y, game_state)[2]):
        bomb_sense=1
    else:
        bomb_sense=0
    return bomb_sense
...
def feature1(own_x: int, own_y: int, game_state: dict) -> torch.tensor:
    ...
    a5= bomb_sense(own_x, own_y, game_state)*4+1
    a6= wait_sense(own_x, own_y, game_state)
