...
class SimpleNet(nn.Module):
    def __init__(self, input_sizes, hidden_layer_sizes, output_size, slope):
...
def setup_training(self):
    ...
    self.hidden_layer_sizes = [56, 168]  # Adjust hidden layer size
    self.output_size = len(ACTIONS)  # Number of output values
    self.model = SimpleNet(self.input_sizes, self.hidden_layer_sizes, self.output_size, self.slope)
    self.target_model = SimpleNet(self.input_sizes, self.hidden_layer_sizes, self.output_size, self.slope)
...
def train_KILLED_OPPONENT (self, new_game_state: dict, old_state: list, new_state: list):
    ...
    for i in range(len(self.memory) - min(last, len(self.memory)), len(self.memory), 1):
        old_state, action, reward, new_state = self.memory[i]
        if not (old_state[4]==1 and (action== 4 or action== 5)):
            self.memory[i] = (old_state, action, reward + add_reward, new_state)
            self.game_reward += add_reward
            if action==4:
