...
class SimpleNet(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        masked_input[zero_mask] = 0.00
        
        final_output=self.layers(masked_input)
...
def train(self, mini_sample):
    ...
    #print("old_states_batch", old_states_batch)
    ...
    for old_state, action, reward, new_state in zip(old_states, actions, rewards, new_states):
        norm_reward= (reward-mean)/(scaling_factor*(std+0.000000001))
...
def setup_training(self):
    ...
    self.hidden_layer_sizes = 12  # Adjust hidden layer size
