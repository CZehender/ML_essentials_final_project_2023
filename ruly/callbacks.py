...
def check_coin(_x: int, _y: int, tile_x: int, tile_y: int, game_state: dict) -> float:
    if not math.isnan(calculate_gravD_coins (_x, _y, game_state)[2]):
        tile_x= calculate_gravD_coins (tile_x, tile_y, game_state)[2]
        tile_y= calculate_gravD_coins (tile_x, tile_y, game_state)[3]
        obj_x= calculate_gravD_coins (_x, _y, game_state)[2]
        obj_y= calculate_gravD_coins (_x, _y, game_state)[3]
        if np.sqrt((tile_x) ** 2 + (tile_y) ** 2)<np.sqrt((obj_x) ** 2 + (obj_y) ** 2):
        ...
        if not math.isnan(calculate_gravD_coins (_x, _y, game_state)[0]):
            tile_x= calculate_gravD_coins (tile_x, tile_y, game_state)[0]
            tile_y= calculate_gravD_coins (tile_x, tile_y, game_state)[1]
            obj_x= calculate_gravD_coins (_x, _y, game_state)[0]
            obj_y= calculate_gravD_coins (_x, _y, game_state)[1]
            if np.sqrt((tile_x) ** 2 + (tile_y) ** 2)<np.sqrt((obj_x) ** 2 + (obj_y) ** 2):
...
def check_others(_x: int, _y: int, tile_x: int, tile_y: int, game_state: dict) -> float:
    if not math.isnan(calculate_gravD_others (_x, _y, game_state)[0]):
        tile_x= calculate_gravD_others (tile_x, tile_y, game_state)[0]
        tile_y= calculate_gravD_others (tile_x, tile_y, game_state)[1]
